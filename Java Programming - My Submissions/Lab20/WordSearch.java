...
public class WordSearch {
	...
	static void ConvertColumnToUpper(char[][] puzzle, int column, int start, int end) {
		for (int n = start; n < end; n++) {
			puzzle[n][column] = ToUpper(puzzle[n][column]);
		}
	}
	...
	static boolean SearchDown(char[][] puzzle, String word) {
		...
		for (int column = 0; column < D; column++) {
			String thisColumn = "";
			for (int row = 0; row < D; row++) {
				thisColumn += puzzle[row][column];
			}
			final int indx = thisColumn.indexOf(word);
			if (indx >= 0) {
				// convert the chars to uppercase
				ConvertColumnToUpper(puzzle, column, indx, indx + word.length());
				return true;
			}
		}
		return false;
